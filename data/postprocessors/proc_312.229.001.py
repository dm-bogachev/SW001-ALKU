...
def process(frame, yolo_data: list):
    ...
    for data in yolo_data:
        ...
        width = (x2 - x1) / 2
        height = (y2 - y1) / 2
        rect_left = int(x1 - 0.9*width)
        rect_right = int(x2 + 0.9*width)
        rect_top = int(y1+0.1*height)
        rect_bottom = int(y2-0.1*height)
        ...
        if not has_intersection:
            if data.class_id != 0:
                cv2.line(drawframe, (rect_left, rect_top), (rect_right, rect_top), (0, 255, 0), 5)
                cv2.line(drawframe, (rect_left, rect_bottom), (rect_right, rect_bottom), (0, 255, 0), 5)
            else:
                cv2.line(drawframe, (rect_left, rect_top), (rect_right, rect_top), (0, 0, 255), 5)
                cv2.line(drawframe, (rect_left, rect_bottom), (rect_right, rect_bottom), (0, 0, 255), 5)
