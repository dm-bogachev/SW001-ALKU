...
class Robot:
    ...
    def __process_loop(self):
        while True:
            try:
                ...
                time.sleep(Config.get("RS007L.PingIntervalMS", 15) / 1000)
            except Exception as e:
                logger.error(f'Ошибка в цикле обработки IO: {e}')

    def __get_data_from_redis(self):
        try:
            data = redis.get(f"IO.RS013N.Output")
            if data:
    ...
    def __set_data_to_redis(self):
        try:
            ...
            redis.set(f"IO.RS007L.Output", data_str)
        except Exception as e:
    ...
    def __connect(self):
        while True:
            ...
            self.__socket.bind((Config.get("RS007L.Host"), Config.get("RS007L.Port")))
            ...
            while True:
                try:
                    ...
                    if self.__tcp_attempts >= Config.get("RS007L.MaxTCPAttempts", 5):
            ...
            if self.__connected:
                ...
                Config.set("RS007L.RobotIP", self.__robotIP[0])
                Config.save()
                break

    def __init__(self):
        
        self.__inputs = [0] * int(Config.get("RS007L.InputSize", 256)/16)
        self.__outputs = [0] * int(Config.get("RS007L.OutputSize", 256)/16)
        self.__internal = [0] * int(Config.get("RS007L.InternalSize", 256)/16)
