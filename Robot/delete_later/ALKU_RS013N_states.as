...
N_INT26    "di.hold|"
N_INT27    "s.allow.put|"
N_INT100    "s.opt.placed|"
N_INT101    "s.ot.placed|"
N_INT102    "do.work[2]|"
N_INT103    "s.tare.full|"
.END
.INTER_PANEL_D
0,9,1,6,15
...
.PROGRAM state1 () ; Wait for program start
  ...
  ;
.END
.PROGRAM state2 () ; Check if required to change gripper
  CALL log ("State 2: Check gripper")
  CALL check.gripper
  state = 3
  ;
.END
.PROGRAM state3 () ; Pick OT from stocker
  CALL log ("State 3: Pick OT")
  ...
  ;
.END
.PROGRAM state4 () ; Pick OPT from stocker
  CALL log ("State 4: Pick OPT")
  ; Check all start positions
  POINT .current.pos = #current.pos
  POINT .stz.wait = #stz.wait
  POINT .pos.wait = #pos.wait
  ...
  ;
.END
.PROGRAM state5 () ; Return OT to stocker
  ...
  ;
.END
.PROGRAM state6 () ; Return OPT to stocker
  ...
  ;
.END
.PROGRAM state7 () ; Pick detail
  CALL log ("State 7: Take detail")
  ; Check all start positions
  POINT .current.pos = #current.pos
  POINT .stz.wait = #stz.wait
  POINT .pos.wait = #pos.wait
  ...
  ;
.END
.PROGRAM state8 () ; Put detail
  CALL log ("State 8: Put detail")
  ; Check all start positions
  POINT .current.pos = #current.pos
  POINT .stz.wait = #stz.wait
  POINT .pos.wait = #pos.wait
  ...
  ;
.END
.PROGRAM state9 () ; Decide if pick next OT
  CALL log ("State 9: Decide if pick next OT")
  ...
  END
.END
.PROGRAM state10 () ; Decide if pick next OPT
  CALL log ("State 10: Decide if pick next OPT")
  ...
  END
.END
.PROGRAM state25 () ; Auxilary state
  CALL log ("State 25: Calculating next step")
  state = 26
  ;
.END
.PROGRAM state26 () ; Decision making
  ...
  IF SIG (s.ot.placed) AND SIG (s.opt.placed) AND NOT SIG (s.grip.full) THEN
    $action = "WaitForPick"
    IF hmi.x >= 0 AND $cycle.command == "PICK" AND NOT SIG (rs7.tare.chg) THEN
      $cycle.command = ""
      state = 7
    END
  END
  ...
  IF SIG (s.ot.placed) AND SIG (s.opt.placed) AND SIG (s.grip.full) THEN
    $action = "WaitPosFree"
    IF NOT SIG (rs7.work[1]) AND SIG (s.allow.put) THEN ; NOT SIG (rs7.working) AND
      state = 8
    END
  END
  ;
  ...
  END
  ;
  IF $cycle.command == "NOPICK" OR count.pick == detail.count THEN
    $cycle.command = ""
    state = 6
  END
...
.PROGRAM pos.put (.pos)
  ...
  ;
  ;SIGNAL -rs13.det.put
  .$temp = "Put detail to positioner (ID:" + $ENCODE (.pos) + ")"
  CALL log (.$temp)
...
.PROGRAM stz.take ()
  ;
  .$temp = "Pick detail from (X:" + $ENCODE (/L, hmi.x) + ", Y:" + $ENCODE(/L, hmi.y) + ", A:" + $ENCODE(/L, hmi.a) + ")"
  CALL log (.$temp)
  ;
  SPEED 100 ALWAYS
  ACCURACY 100 ALWAYS
...
.PROGRAM ot.take (.i,.j)
  IF FALSE THEN
    ...
    .j = hmi.st.out.j
  END
  .$temp = "Take OT with (i: " + $ENCODE (/L, .i) + ", j: " + $ENCODE (/L, .j) + ")"
  CALL log (.$temp)
